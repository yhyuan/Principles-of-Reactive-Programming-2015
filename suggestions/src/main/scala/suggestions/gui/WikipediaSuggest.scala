...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.concatRecovered(term => ObservableEx(wikipediaSuggestion(term)).timedOut(10))
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map(button => suggestionList.selection.items.headOption).filter(_.nonEmpty).map(_.get)

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(selection => ObservableEx(wikipediaPage(selection)).timedOut(10))
